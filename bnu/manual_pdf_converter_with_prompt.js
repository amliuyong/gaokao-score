/**
 * Manual PDF to Image Converter with Optimized Prompt
 * 
 ...
 * Includes an optimized prompt for more accurate data extraction.
 */
...
const getOptimizedPrompt = () => {
  return `我需要你分析这些图片中的北京师范大学2024年陕西省招生计划和录取数据表格。
...
async function main() {
  try {
    ...
    await fs.writeFile(promptPath, getOptimizedPrompt(), 'utf8');
    ...
    console.log('5. Set the system prompt to:');
    console.log('   "你是一位专业的数据提取专家，擅长从表格图片中提取结构化数据并按照指定格式输出。你的分析非常精确，不会遗漏任何表格行，也不会添加任何主观解释。"');
